  ...
  i_seq <-seq(0,100,10)
   ...
   grid_search$score <- 0
    ...
    pam50_clusters <- metadata %>%
        as.data.frame() %>%
        filter(rownames(metadata) %in% rownames(dat$test_metadata)) %>%
    ...
    scores <- lapply( 1:nrow(grid_search), function(i) {
        ...
        both_clusters <- both_clusters %>%
                filter(!is.na(top) & !is.na(paper_BRCA_Subtype_PAM50))
    ...
    both_clusters <- both_clusters %>%
        ...
        grnmf_res = ARI(as.factor(both_clusters$paper_BRCA_Subtype_PAM50), as.factor(both_clusters$top))
...
save(results, file = "TCGA_breast_Independent_to_PAM50_results.RData")
...
## Top best performing score with graph regularization is lambda_1 = 100, lambda_2 = 6
...
pam50LVs <- grackle$H
rownames(pam50LVs) <- c("LumA_1", "Normal", "Basal", "LumB", "LumA_2")
colnames(pam50LVs) <- colnames(train_expression)
    
write.csv(as.data.frame(t(pam5LVs)), file = "PAM50_gene_LVs_Independent_sim.csv")
