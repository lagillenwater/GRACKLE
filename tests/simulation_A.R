...
g <- randomNetwork(prior_graph = breast_g, num_nodes = 200)
...
for(x in t_sequence) {
    for(y in nn_sequence) {
        ...
        if(x == 0) {
            ...
            print(paste0("T: ", round(transitivity(new_g,type = "global"),4)))
...
#        print(membership_counts)
        ...
        print(paste("nn_sequence", y))

        if(y !=0) {
            new_g <- networkNoise(g,goal_modularity = y, membership = membership,large_clusters = large_clusters)
        }

        print(paste("M:", round(modularity(cluster_louvain(as.undirected(new_g), weights = NA)), 4)))
        cat("\n")
