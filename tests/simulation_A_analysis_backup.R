...
full_results <- lapply(results_list, function(x) {
  ...
  tmp <- lapply(x, function(y){
    ...
    avg_grackle <- lapply(grid_searches, function(z) {
      z %>%
        filter(!(lambda_1 ==0 & lambda_2 ==1) | (lambda_1 ==0 & lambda_2 ==0))%>%
...
top_results$name <-  factor(top_results$name, levels = c( "avg_grackle","top_grackle", "gnmf_results", "pr_nmf_results","nmf_results" ))

p1 <- ggplot(top_results, aes(x = as.factor(expression_noise), y = value, fill = name)) +
...
ggsave(p1, filename = "../GRACKLE_results/violin_overall.pdf")
...
for(i in seq(0.3,.7,.2)){
  ...
  ggsave(p1, filename = paste0("../GRACKLE_results/mod_violin_noise_",i,".pdf"))
...
for(i in seq(.3,.7,.2)){
  ...
  ggsave(p1, filename = paste0("../GRACKLE_results/trans_violin_noise_",i,".pdf"))
