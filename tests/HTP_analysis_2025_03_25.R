...
file.list <- list.files("./results", pattern ="HTP_results_updated_1_23", full.names = T )
...
gnmf.list <- list.files("./results_2025_03_25", pattern ="HTP_results", full.names = T )

gnmf_results <- lapply(gnmf.list[c(1,2,3)],function(x) get(load(x)))

gnmf_results <- lapply(gnmf_results, function(x) {
  x[18] <- NULL
  return(x)
})


all_results <- lapply(1:length(results), function(x) {
    grid_searches <- lapply(results[[x]], function(y) {y[[1]]})
    ...
    gnmf_result <- lapply(gnmf_results[[1]], function(a) {
      a$grnmf_res %>% 
        filter(score == max(score)) %>%
        distinct(score)
    })
