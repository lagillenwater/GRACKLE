...
for ( x in 1:50) {
  ...
  dat$test_expression <- as.matrix(min_max_scale(dat$test_expression,min_vals,max_vals))
    ...
    for (a in grnmf_res$alpha) { 
        ...
        tmp_res= ARI(as.factor(both_clusters$paper_BRCA_Subtype_PAM50), as.factor(both_clusters$top))
        print(paste("GRNMF score for alpha", a, " = ", tmp_res))
        grnmf_res[grnmf_res$alpha == a, "score"]  = tmp_res
    }
        
    results[[x]] <- list(grnmf_res = grnmf_res)
    save(results, file = paste0("TCGA_breast_with_methylation_results_GNMF_", Sys.Date(), ".RData"))
